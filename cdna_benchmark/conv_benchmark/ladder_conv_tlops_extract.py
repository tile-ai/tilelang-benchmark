...
def extract_tflops(data):
    ...
    best_tflops_pattern = r"best latency:\s*([\d\.]+)"
    ref_tflops_pattern = r"Best TFlops:\s*([\d\.]+)"
    ...
    best_tflops = float(best_tflops_match.group(1)) if best_tflops_match else 100000.0
    ref_tflops = float(ref_tflops_match.group(1)) if ref_tflops_match else 100000.0

    return best_tflops, ref_tflops

id = 0
for n, c, h, w, f, k, s, d, p in shapes:
    log_name = f"./logs/ladder/conv_{n}_{c}_{h}_{w}_{f}_{k}_{s}_{d}_{p}.log"
    id += 1
    with open(log_name, "r") as f:
        data = f.read()
        best_latency, best_tflops = extract_tflops(data)
    print(best_latency, best_tflops)