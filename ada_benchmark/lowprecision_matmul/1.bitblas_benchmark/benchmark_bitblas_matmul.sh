...
dtypes=(
    ...
    "e4m3_float8 e4m3_float8 float32 float32"
    "e5m2_float8 e5m2_float8 float32 float32"
...
for shape in "${shapes[@]}"; do
    ...
    for dtype_combo in "${dtypes[@]}"; do
        ...
        cmd="python3 ./benchmark_bitblas_matmul.py --M ${m} --N ${n} --K ${k} --A_dtype ${A_dtype} --W_dtype ${W_dtype} --out_dtype ${out_dtype} --accum_dtype ${accum_dtype}"
