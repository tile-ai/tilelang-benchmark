...
def _attn_fwd_inner(acc, l_i, m_i, q, block_mask,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        mask_val = tl.load(block_mask + start_n * stride_bn)
        if mask_val == True:
            # -- compute qk ----
...
@triton.jit
def _attn_fwd_inner_tma(acc, l_i, m_i, q, block_mask,  #
                        desc_k, desc_v,  #
...
@triton.jit
def _attn_fwd_tma(sm_scale, M, block_mask,  #
    ...
    # For causal = False, STAGE = 1, and _attn_fwd_inner gets 3 as its STAGE
    if STAGE & 1:
        acc, l_i, m_i = _attn_fwd_inner_tma(acc, l_i, m_i, q, block_mask,  #
                                            desc_k, desc_v,  #
